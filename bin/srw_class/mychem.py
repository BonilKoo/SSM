...
class mychem():
    ...
    def cal_T(self, molobj, molgraph, smiles, chemistry="graph"):
        ...
        return T
    # END of cal_T
    def cal_path_df(molgraph, mol_T, n_walker, walkLength=10, mode = "argmax", parallel=False):
        # n_walker: dictionary {node1: count, node2: count, ...}
        ...
        for node_idx in range(nNodes):
            ...
            z = np.zeros(nNodes)
            p = z
            p[node_idx] = 1 # probability
            p = p.reshape(-1,1)
            for n_walk in range(n_walker[node_idx]):
                dVisited[node_idx][n_walk] = [node_idx]
                for k in range(walkLength):
                    neigh = [x for x in molgraph.neighbors(dVisited[node_idx][n_walk][-1])]
                    p = np.dot(mol_T, p) # redefine transition probability
                    if mode == 'argmax':
                    ...
                    dVisited[node_idx][n_walk].append(visit)
        return dVisited
    #
#     def _cal_path_df(molgraph, mol_T, walkLength=10, n_walker=100, mode = "argmax", parallel=False):
#         nNodes = molgraph.number_of_nodes()
...
#         return pdPath
    ...
    def rwr_summary(mol_graph, rw_result_dict, n_walker):
        # n_walker: self.dNodeSubgraphCount[n_iter][ltkbid] dictionary
        ...
        for node in rw_result_dict:
            ...
            for walker in range(n_walker[node]):
