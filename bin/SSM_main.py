...
class DILInew:
    def __init__(self, chemistry = 'graph', n_rw=10, n_alpha = 0.5, iteration = 10, pruning=False, n_walker=100, rw_mode = "argmax"):
        ...
        self.dEdgeUsedCount  = defaultdict(dict) # number of visited edges by RW for each graph; key: iteration, subkey: ltkbid, value: {edge_1:n_1, edge_2:n_2, ...}
        self.dEdgelistUsage  = defaultdict(dict) # key: iteration, subkey: ltkbid, value: {key: node, values: defaultdict(key 1: 'node_list', value 1: nodes (a list of lists) / key2: 'edge_list', value 2: edges (a list of lists))}
        self.dNodeFragCount  = defaultdict(dict) # fragment dictionary for each molecule; key: iteration, subkey: ltkbid, value: {frag_1:n_1, frag_2:n_2, ...}
        self.dNodeFragSmiles = defaultdict(dict) # key: iteration, subkey: ltkbid, value: {key: node (int), value: a list of fragments (list)}
        #
        self.dEdgeClassDict = {} # summary for class; Doublet preference; used only for training; key: iteration, subkey: fragment, value: dict(key: class, value: count (sum of the values in dEdgeUsedCount of the edges in the fragment))
        #
        self.dMolTransDict  = defaultdict(dict) # key: iteration, value: {ltkbid_0:T_0, ltkbid_1:T_1, ..., ltkbid_N:T_N} # ltkbid: 'ID', T: transition matrix of graph
        ...
        #
        self.lexclusivefrags = defaultdict(list) # key: iteration, value: a list of fragments that exist in only one class; used only for training
        self.lunionfrags     = defaultdict(list) # key: iteration, value: a list of fragments that exist in both class; used only for training
        # self.dFragSearch     = {}
        # self.prunehistory    = defaultdict(dict)

    def DoRandomWalk(self, n_iter, ltkbid, T): # run_rw starts here
        dpaths = mychem.cal_path_df(self.molinfo_df["molgraph"][ltkbid], T, walkLength = self.n_rw, n_walker = self.n_walkers, mode = self.rw_mode ) # index: walker (int), column: atomID (int), value: visited nodes (list)
    ...
    def rw_update_transitions(self, _T, _F, update_alpha):
        _T = _T * (1-update_alpha) + update_alpha * _F # (Equation 1)
    ...
    def get_fraglist(self, n_iter):  # Get list of exclusive fragments
        ...
        tempdf = pd.DataFrame(self.dNodeFragCount[n_iter], columns= self.dNodeFragCount[n_iter].keys()).fillna(0).T # row: ltkbid, column: fragment, value: count
        ...
        lExList = nClassSpecificity[nClassSpecificity].index.to_list() # Fragments that exist in only one class
        nClassSpecificity  = dfTempCnt.groupby('class').any().astype(int).sum(axis=0)>0
        lUnionList = nClassSpecificity[nClassSpecificity].index.to_list() # Fragments that exist in both classes
    ...
    def train(self, train_data):
        ...
        for nI in range(self.n_iteration):  # iterate random walk process
            ...
            for ltkbid in self.molinfo_df["ID"]: # iterate over molecules
                ...
                if nI == 0: # cal_T(molobj, molgraph, smiles, chemistry='graph') # first iteration
                    T = mychem.cal_T(mychem, self.molinfo_df["molobj"][ltkbid], self.molinfo_df["molgraph"][ltkbid], smiles, chemistry = self.chemistry) # transition matrix of graph
                else:
                    pd_pref =  pd.DataFrame( self.dEdgeClassDict[nI-1], columns = self.dEdgeClassDict[nI-1].keys() ).fillna(0) # row: class, column: fragment, value: count
...
class analyze_individual():
    def __init__(self):
        self.frag_df = defaultdict(pd.DataFrame) # key: iteration, value: pd.DataFrame (row: ltkbid, column: fragment, value: count)
        self.lfraglist = defaultdict(list) # key: iteration, value: a list of fragments
...
def prediction(train_obj, valid_obj, nIter, output_dir, train_molinfo_df, valid_molinfo_df, n_seed = 0): # train.pickle, test.pickle
	...
	# if (nI + 1) == nIter:
		...
		if 'class' in valid_molinfo_df.columns:
		    ...
		    result_list = [n_union, n_train, n_valid, accuracy, balanced_accuracy, precision, recall, f1, roc_auc, mcc]
